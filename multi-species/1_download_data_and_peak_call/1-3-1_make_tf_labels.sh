...
#--------------------------------------------------
# Called by `1-3_create_full_dataset.sh`
#--------------------------------------------------
...
peakcall_out_dir="$DATA_DIR/${genome}/${tf}"
genome_windows_file="$DATA_DIR/${genome}/windows.bed"  # output from make_windows_files.sh
peak_call_file="$peakcall_out_dir/mgps_out_${tf}.bed"  # output from peak calling
...
# The tmp files (will be deleted at the end of the script)
peak_windows_file="$peakcall_out_dir/peaks_${windowSize}bp.bed"
genome_windows_with_peaks_sorted_file="$peakcall_out_dir/peaks_${windowSize}bp.sorted.bed"
pos_labels_file="$peakcall_out_dir/binding_labels.pos.bed"
neg_labels_file="$peakcall_out_dir/binding_labels.neg.bed"
sorted_genome_windows_file="$DATA_DIR/${genome}/windows.dictsort.bed"

# final output file
labels_file="$peakcall_out_dir/binding_labels.bed"

echo "Getting TF binding peak windows from peak calling output..."

# convert peak calling, peak center coords to windowSize-bp windows, in bed format
