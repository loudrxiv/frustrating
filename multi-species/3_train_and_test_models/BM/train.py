...
from generators import TrainGenerator_MultiSpecies, ValGenerator_MultiSpecies
...
if __name__ == "__main__":
	...
	params  = Params(args = ["BM", tf, target_species], verbose=True)
	print("\n---------------------------------------\n")
	...
	for epoch in range(0, num_epochs):
		...
		for batch_idx, data in train_bar:
			...
			binding_pred = torch.squeeze(basic_model(binding_seq))

			assert binding_pred.shape[0] == binding_seq.shape[0], (binding_pred.shape, binding_seq.shape[0])

			# Measure loss with binary cross entropy and CORAL
			loss_pred = nn.BCELoss()(binding_pred, binding_label.float())
			loss = loss_pred
		...
		with torch.no_grad():
			...
			for batch_idx, data in val_bar:
				for species, path in vg_ms.valfiles.items():
					...
					val_results[species]['probs'].extend(basic_model(curr_data).cpu().numpy().tolist())
			...
			if len(current_auprcs) == 0 or target_auprc > max(current_auprcs):
				...
				if SAVE:
					torch.save(basic_model.state_dict(), f"{params.modelfile}.basic_model.pt")
