...
def get_model_file(tf, test_species):
    ...
    model_path = ROOT + "/".join(["/models", tf, test_species + "_tested", "BM/"])
...
def get_preds_file(tf, test_species):
    preds_root = ROOT + "/model_out"
    os.makedirs(preds_root, exist_ok=True)
    return f"{preds_root}/BM_{tf}_{test_species}-tested.preds"

def get_labels_file(tf, test_species):
    preds_root = ROOT + "/model_out"
    os.makedirs(preds_root, exist_ok=True)
    return f"{preds_root}/BM_{tf}_{test_species}-tested.labels"
...
if __name__ == "__main__":
    ...
    params                  = Params(args = ["BM", tf, target], verbose=True)
    print("\n---------------------------------------\n")
    
    model_file      = get_model_file(tf, target)
    preds_file      = get_preds_file(tf, target)
    labels_file     = get_labels_file(tf, target)

	#---  (2) Define model, optimizer and load weights

    basic_model = BasicModel(params)
    basic_model = basic_model.to(device)
    print(f"BM Model Architecture:\n{basic_model}\n")
