...
if __name__ == "__main__":
    ...
    train_file, tf, target_species, _lambda, _match_mean = sys.argv
    _lambda     = float(_lambda)
    _match_mean = bool(int(_match_mean))

    params  = Params(args = ["MORALE", tf, target_species], verbose=True)
    print("\n------------------------------------------\n")
	...
	# # We load previous weights if we specify it
    # if (SAVE):
    #     print("We are going to train the models from scratch.\n")
    # else:
    #     saved_feature_extractor = f"{ROOT}/models/{sys.argv[1]}/{sys.argv[2]}_tested/CORAL/2024-11-30_10-15-18.feature_extractor.pt"
    #     saved_classifier        = f"{ROOT}/models/{sys.argv[1]}/{sys.argv[2]}_tested/CORAL/2024-11-30_10-15-18.classifier.pt"

    #     print(f"Loading model from {saved_feature_extractor}.\n")
    #     feature_extractor.load_state_dict(torch.load(saved_feature_extractor))

    #     print(f"Loading model from {saved_classifier}.\n")
    ...
    for epoch in range(0, num_epochs):
        ...
        with torch.no_grad():
            ...
            target_auprc = print_val_metrics(target_species, val_results)
            ...
            if len(current_auprcs) == 0 or target_auprc > max(current_auprcs):
                ...
                if SAVE:
                    torch.save(feature_extractor.state_dict(), f"{params.modelfile}.feature_extractor.pt")
                    torch.save(classifier.state_dict(), f"{params.modelfile}.classifier.pt")
