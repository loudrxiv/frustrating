...
def get_preds_file(tf, test_species):
    ...
    return f"{preds_root}/MORALE_{tf}_{test_species}-tested.preds"
...
if __name__ == "__main__":
    ...
    test_file, tf, target, _lambda, _match_mean = sys.argv
    params                                      = Params(args = ["MORALE", tf, target], verbose=True)
    print("\n---------------------------------------\n")

    # Get both model files
    classifier_file, feature_extractor_file = get_model_files(tf, target)
