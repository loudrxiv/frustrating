...
def get_model_files(tf, test_species, num_holdout):
    ...
    model_path = ROOT + "/".join(["/models", tf, test_species + "_tested", f"EvoGS-{num_holdout}/"])
...
def get_preds_file(tf, test_species, num_holdout):
    ...
    return f"{preds_root}/EvoGS-{num_holdout}_{tf}_{test_species}-tested.preds"

def get_labels_file(tf, test_species, num_holdout):
    preds_root = ROOT + "/model_out"
    os.makedirs(preds_root, exist_ok=True)
    return f"{preds_root}/EvoGS-{num_holdout}_{tf}_{test_species}-tested.labels"
...
if __name__ == "__main__":
    ...
    test_file, tf, target, _lambda, match_mean, num_holdout = sys.argv
    ...
    assert num_holdout < len(evolutionary_relatedness), (num_holdout, len(evolutionary_relatedness))
    ...
    classifier_file, feature_extractor_file = get_model_files(tf, target, num_holdout)
    preds_file                              = get_preds_file(tf, target, num_holdout)
    labels_file                             = get_labels_file(tf, target, num_holdout)
